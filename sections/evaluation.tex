...
\subsubsection*{Deduplication}
The approach used for deduplicatingblock devicess is essentially the same as the one for memory dumps.
...
it also works foblock deviceses.
The main difference is the rate of follow-up checkpoints is much lower,
block devicesces generally experience fewer changes than memory regions.
Depending on the use case, this can however differblock devicesices are copied as a whole and not in parts like main memory.